...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(data, values='class', names='index', title=f'Total Success Launches for {entered_site}')
    return fig
...
def get_scatter_chart(entered_site,entered_payload):
    data = spacex_df[spacex_df["Payload Mass (kg)"].between(entered_payload[0],entered_payload[1])]
    if entered_site == 'ALL':
        fig = px.scatter(data, x="Payload Mass (kg)",y="class", color="Booster Version Category")
    else:
        fig = px.scatter(data[data["Launch Site"] == entered_site], x="Payload Mass (kg)", y="class", color="Booster Version Category")
    return fig
